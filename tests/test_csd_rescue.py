...
def _make_self_loop_graph() -> nx.DiGraph:
    """Build a graph with a single self-loop X -> X."""
    g = nx.DiGraph()
    g.add_edge("X", "X")
    return g
...
class TestClassifyNonidentCause:
    """Tests for classify_nonident_cause: taxonomy of non-identifiable cause types."""

    def test_self_loop(self):
        """A self-loop X->X is classified as 'self_loop'."""
        g = _make_self_loop_graph()
        result = classify_nonident_cause(g, "X", "X")
        assert result == "self_loop"

    def test_two_cycle_forward(self):
        """A->B in a 2-cycle A<->B is classified as 'two_cycle'."""
        g = _make_two_cycle_graph()
        result = classify_nonident_cause(g, "A", "B")
        assert result == "two_cycle"

    def test_two_cycle_backward(self):
        """B->A in a 2-cycle A<->B is also classified as 'two_cycle'."""
        g = _make_two_cycle_graph()
        result = classify_nonident_cause(g, "B", "A")
        assert result == "two_cycle"

    def test_same_scc_long(self):
        """An edge A->C where removing it still leaves A and C in the same SCC is 'same_scc_long'."""
    ...
    def test_scc_edge_dissolved(self):
        """An edge A->B whose removal dissolves the SCC is classified as 'scc_edge_dissolved'."""
    ...
    def test_cross_scc_blocked(self):
        """An edge E->F where E and F are in different SCCs is classified as 'cross_scc_blocked'."""
...
class TestComputeRescueNodes:
    """Tests for compute_rescue_nodes: finding interventions that rescue identifiability."""
