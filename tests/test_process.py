...
def test_generate_test_data_tensor_custom_params():
    ...
    tensor = generate_test_data_tensor(n_rows=50, n_cols=100, scale_dist_params=(1, 0.5), loc_dist_params=(3, 25))
...
def test_apply_lognormal_noise_process_different_noise():
    ...
    assert not torch.equal(noisy_tensor1, noisy_tensor2), "Noisy tensors are equal for different noise applications"
...
def test_apply_lognormal_noise_process_mean_std():
    ...
    assert torch.isclose(log_noise.mean(), torch.tensor(0.0), atol=0.1), "Mean of noise is not close to expected mean"
    assert torch.isclose(log_noise.std(), torch.tensor(1.0), atol=0.1), "Standard deviation of noise is not close to expected std"
...
def test_apply_bernoulli_lognormal_outlier_process_different_mu_sigma():
    ...
    assert not torch.equal(outlier_tensor1, outlier_tensor2), "Outlier tensors are equal for different mu and sigma values"
...
def test_apply_bernoulli_lognormal_outlier_process_outlier_range():
    ...
    assert outlier_tensor.min().item() >= expected_min, f"Outlier tensor contains values below expected range: {outlier_tensor.min().item()} < {expected_min}"
    assert outlier_tensor.max().item() <= expected_max, f"Outlier tensor contains values above expected range: {outlier_tensor.max().item()} > {expected_max}"


def test_apply_row_normalization_and_lognormal_scaling_shape():
    """Test shape after normalization and scaling."""
    tensor = generate_test_data_tensor(n_rows=100, n_cols=200)
    normalized_scaled_tensor = apply_row_normalization_and_lognormal_scaling(tensor, mu=0.0, sigma=1.0)
    assert normalized_scaled_tensor.shape == tensor.shape, "Shape mismatch after normalization and scaling"
...
def test_apply_row_normalization_and_lognormal_scaling_no_nan_inf_negative():
    ...
    tensor = generate_test_data_tensor(n_rows=100, n_cols=200)
    normalized_scaled_tensor = apply_row_normalization_and_lognormal_scaling(tensor, mu=0.0, sigma=1.0)
    assert not torch.isnan(normalized_scaled_tensor).any(), "Normalized and scaled tensor contains NaN values"
    assert not torch.isinf(normalized_scaled_tensor).any(), "Normalized and scaled tensor contains Inf or -Inf values"
    assert (normalized_scaled_tensor >= 0).all(), "Normalized and scaled tensor contains negative values"
...
def test_apply_row_normalization_and_lognormal_scaling_different_mu_sigma():
    ...
    tensor = generate_test_data_tensor(n_rows=100, n_cols=200, seed=42)
    normalized_scaled_tensor1 = apply_row_normalization_and_lognormal_scaling(tensor, mu=0.0, sigma=1.0)
    normalized_scaled_tensor2 = apply_row_normalization_and_lognormal_scaling(tensor, mu=1.0, sigma=2.0)
    assert not torch.equal(normalized_scaled_tensor1, normalized_scaled_tensor2), "Normalized and scaled tensors are equal for different mu and sigma values"
...
def test_apply_quantile_logistic_dropout_process_different_params():
    ...
    assert not torch.equal(dropout_tensor1, dropout_tensor2), "Dropout tensors are equal for different k values"
    dropout_tensor3 = apply_quantile_logistic_dropout_process(tensor, k=1.0, q=0.7)
    assert not torch.equal(dropout_tensor1, dropout_tensor3), "Dropout tensors are equal for different q values"
...
def test_apply_quantile_logistic_dropout_process_expected_range():
    ...
    assert dropout_tensor.max().item() <= tensor.max().item(), "Dropout tensor contains values above original tensor max"
...
def test_apply_quantile_logistic_dropout_process_more_dropouts():
    ...
    assert dropout_nonzero_count < original_nonzero_count, "There are not more dropouts after applying the process"
...
def test_apply_poisson_process_different_inputs():
    ...
    assert not torch.equal(poisson_tensor1, poisson_tensor2), "Poisson tensors are equal for different inputs"
