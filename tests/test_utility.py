...
def test_create_sbml_model_from_nx():
    ...
    def test_odes(document, expected_odes):
        ...
        for species_id, expected_ode in expected_odes.items():
            assert expected_ode in odes, f"ODE for {species_id} does not match: Expected {expected_ode}, but found different."
            print(f"ODE for {species_id} matches: {expected_ode}")
