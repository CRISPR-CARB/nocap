...
def main() -> None:
    ...
    parser.add_argument("--demo", choices=("direct", "chain", "cycle", "small_network", "frontdoor_cycle"), default="cycle")
    parser.add_argument("--treatment", required=True)
    parser.add_argument("--outcome", required=True)
    parser.add_argument("--treatment-levels", default="-1,1")
    parser.add_argument(
        "--outcome-bounds",
        required=True,
    ...
    query = identify_query_status(graph, args.treatment, args.outcome)
    if not query.identifiable or query.expression is None:
        raise SystemExit(f"query is unidentifiable: {query.error}")
    nodes = sorted(graph.nodes())
    build = build_synthetic_scm(graph, nodes, missing_edge_rate=args.missing_edge_rate, beta_med=args.beta_med, beta_log_sd=args.beta_log_sd, beta_p=args.beta_p, beta_abs_max=args.beta_abs_max, rng=np.random.default_rng(args.seed))
    ...
    interventions = simulate_intervention_levels(build.scm, config, args.treatment, levels, seed=args.seed + 2)
    ...
    density_ate = estimate_ate(query.expression, observational_data, args.treatment, args.outcome, (levels[0], levels[1]), mode="continuous", backend=SciPyGaussianKDE(), outcome_bounds=bounds)
    density = evaluate_probability_expression(
        query.expression,
        observational_data,
        mode="continuous",
        backend=SciPyGaussianKDE(),
        bounds={args.outcome: bounds},
    )
    if args.outcome not in density.variables:
        raise ValueError("identified expression does not contain the requested outcome")
    grid_lower = bounds[0] if np.isfinite(bounds[0]) else float(observational_data[args.outcome].min())
    grid_upper = bounds[1] if np.isfinite(bounds[1]) else float(observational_data[args.outcome].max())
    ...
    true_ate = true_scm_ate(build.scm, args.treatment, args.outcome, (levels[0], levels[1]), seed=args.seed + 3)
    ...
    result = {"treatment": args.treatment, "outcome": args.outcome, "data_scale": "latent_log_expression", "treatment_levels": levels, "outcome_bounds": [str(bound) for bound in bounds], "n_samples": args.n_samples, "identified_expression": str(query.expression), "true_ate": true_ate, "identified_distribution_ate": density_ate, "identified_ate_error": density_ate - true_ate, "estimated_scm_ate": estimated_ate_value, "estimated_scm_ate_error": estimated_ate_value - true_ate, "estimated_scm_edges": list(estimated_scm.graph.edges()), "added_hidden_edges": build.added_edges, "intervention_treatment_values": {str(level): float(state.latent_log_expression[0, state.scm.nodes.index(args.treatment)]) for level, state in interventions.items()}}
    output.with_suffix(".edges.json").write_text(
        json.dumps(
            [
                {
                    **estimate.__dict__,
                    "true_coefficient": build.scm.betas.get(
                        (estimate.cause, estimate.effect)
                    ),
                }
