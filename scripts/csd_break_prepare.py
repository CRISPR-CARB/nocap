"""csd_break_prepare.py — Shard unidentifiable edges for the SCC-break sweep.
...
This is the prepare step for the *new* csd_break_* pipeline, which computes
the minimum vertex-intervention set B (via min_scc_break_set) that severs the
residual SCC containing cause and effect after removing the direct edge.

Usage
-----
    python scripts/csd_break_prepare.py \\
        --results-csv notebooks/Ecoli_Analysis_Notebooks/csd_results.csv \\
        --shard-dir results/cyclic_single_door/break_shards \\
        --manifest results/cyclic_single_door/break_manifest.json \\
        --shard-size 50
"""
...
def main() -> None:
    ...
    print(f"break prepare: {len(edges)} unidentifiable edges to process.")
    ...
    print(f"break prepare: {len(shards)} shards written to {shard_dir}")
    print(f"break prepare: manifest → {manifest_path}")
