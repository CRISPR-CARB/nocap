...
def load(path):
    with open(path) as f:
        return {(r["cause"], r["effect"]): r["status"] for r in csv.DictReader(f)}
...
regressions = [
    (k, old[k], new[k]) for k in old if old[k] not in ("timeout",) and k in new and new[k] != old[k]
]
resolved = [(k, new[k]) for k in old if old[k] == "timeout" and k in new and new[k] != "timeout"]
still_to = [k for k in old if old[k] == "timeout" and (k not in new or new[k] == "timeout")]
